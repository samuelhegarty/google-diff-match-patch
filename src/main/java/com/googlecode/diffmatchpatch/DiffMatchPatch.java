...
public class DiffMatchPatch{
	 ...
	 */
	private final float diffTimeout;// = 1.0f;
	/**
	 * Cost of an empty edit operation in terms of edit characters.
	 */
	private final short diffEditCost;// = 4;
	/**
	 * At what point is no match declared (0.0 = perfection, 1.0 = very loose).
	 */
	private final float matchThreshold;// = 0.5f;
	/**
	 * How far to search for a match (0 = exact location, 1000+ = broad match).
	 * A match this many characters away from the expected location will add
	 * 1.0 to the score (0.0 is a perfect match).
	 */
	private final int matchDistance;// = 1000;
	/**
	 ...
	 */
	private final float patchDeleteThreshold;// = 0.5f;
	/**
	...
	protected int matchBitap(String text, String pattern, int loc){
		assert (MATCH_MAX_BITS == 0 || pattern.length() <= MATCH_MAX_BITS)
	...
	public Object[] patchApply(LinkedList<Patch> patches, String text){
		...
		for(Patch aPatch : patches){
			...
			int start_loc;
			int end_loc = -1;
			if(text1.length() > this.MATCH_MAX_BITS){
				// patchSplitMax will only provide an oversized pattern in the case of
				// a monster delete.
				start_loc = matchMain(text,
						text1.substring(0, this.MATCH_MAX_BITS), expected_loc);
				if(start_loc != -1){
					end_loc = matchMain(text,
							text1.substring(text1.length() - this.MATCH_MAX_BITS),
							expected_loc + text1.length() - this.MATCH_MAX_BITS);
			...
			if(start_loc == -1){
				...
				if(end_loc == -1){
					...
					text2 = text.substring(start_loc,
							Math.min(end_loc + this.MATCH_MAX_BITS, text.length()));
				}
				if(text1.equals(text2)){
